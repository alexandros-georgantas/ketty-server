...
const { bookComponentCreator } = require("./utils/createBookComponent");

const {
  bookComponentContentCreator,
} = require("./utils/bookComponentContentCreator");
...
const { createDivision } = require("../../api/useCases/division");
const {
  createTeam,
  getEntityTeams,
  deleteTeam,
} = require("../../api/useCases/team");
  ...
  try {
    ...
    return useTransaction(
      async (tr) => {
        ...
        logger.info(
          `>>> book with id ${bookId} patched with the new divisions`
        );
        ...
        logger.info(
          `>>> creating TOC component for the book with id ${bookId}`
        );
