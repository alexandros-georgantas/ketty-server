...
} = require("../../models/book/utils/bookComponentContentCreator");
...
const unlockBookComponent = async (
  ...
  try {
    ...
    return useTransaction(async (tr) => {
      ...
      logger.info(
        `lock for book component with id ${bookComponentId} deleted `
      );
...
const updateWorkflowState = async (bookComponentId, workflowStages, ctx) => {
  try {
    ...
    if (locks.length > 0) {
      const currentBookComponent = await BookComponent.findById(
        bookComponentId
      );
    ...
    logger.info(
      `book component state with id ${bookComponentState.id} updated`
    );
